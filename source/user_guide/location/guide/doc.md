...
1. 测试时没有显示紫色数据
  ...
  3. 检查车载定位标签的数据线是否插在 USB-Hub 的指定接口上，Hub 指示灯是否亮着，数据线是否插紧
  4. 检查 USB-HUB 的数据线是否插在主控单元主机的指定接口上
  5. 打开 terminal，执行以下命令内容，查看是否有 "hub_7" 这个设备
...
看不到激光雷达数据？
  ...
  1. 检查激光雷达是否插在 USB-Hub 上指定的 Hub 口上，Hub 指示灯是否亮着，数据线是否插紧
  3. 检查 USB-HUB 的数据线是否插在主控单元主机的指定接口上
  4. 打开 terminal，执行以下命令内容，查看是否有 "hub_4" 与 "hub_5" 这两个设备
...
程序打开RVIZ后，可以看到界面中显示了 PM1 模型代表 PM1，红色的点为前置激光雷达数据，黄色的点为后置激光雷达数据，紫色的路线为录制的轨迹。

注：即使用户从未录制轨迹，首次打开程序界面上也会显示一条轨迹，此为系统预置轨迹，用户录制轨迹后，此默认轨迹会被覆盖。

<img width="80%" src="imgs/location-2.png"/>

使用手柄控制 PM1 遥控走一段路，如从未录制过轨迹，在走的过程中 PM1 在RVIZ 中会持续跳动，此时 PM1 是在进行地图坐标匹配，当 PM1 不再跳动时，则表示稳定， 走3-5m，边走边转动，障碍物信息不再变化（跳），如下图所示。
